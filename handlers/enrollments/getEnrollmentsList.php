...
try {
    ...
    if ($role === ROLE_ADMIN) {
        $result = $enrollment->getEnrollments(null, null, $page, $limit);

    } elseif ($role === ROLE_INSTRUCTOR) {
        $result = $enrollment->getEnrollments('instructor_id', $userId, $page, $limit);

    } else {
        $result = $enrollment->getEnrollments('student_id', $userId, $page, $limit, ['requested', 'withdrawn'], true);
    }

    echo json_encode(Result::success('Enrollments fetched', [
